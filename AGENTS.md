# Agent Guidelines for Spec Driven Asteroids

This document provides instructions and context for AI coding agents (and human developers) working on the `spec-driven-asteroids` repository. This project is a monorepo that builds tools to inject specification-driven development workflows into other projects.
...
pnpm --filter @spec-driven-asteroids/mcp build

# Build only the CLI package
pnpm --filter spec-driven-asteroids build
```
...
- **`packages/cli`**: The command-line interface (`spec-driven-asteroids`).
