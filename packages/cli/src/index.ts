...
function getMcpChoices() {
  const categories = Array.from(new Set(MCP_SERVERS.map(s => s.category)));

  // Group MCP servers by category
  const mcpChoices = categories.map(category => {
    const serversInCategory = MCP_SERVERS.filter(s => s.category === category);
    return {
      name: chalk.bold(`${category.toUpperCase()} Servers`),
      value: `category-${category}`,
      short: serversInCategory.map(s => `  ${s.name}`).join('\n'),
      disabled: false
    };
  });
...
async function configureCopilotMcp(targetDir: string, selectedMcpIds: string[]) {
  try {
    ...
    // Add spec-driven-asteroids (always)
    config.mcpServers['spec-driven-asteroids'] = {
      command: 'pnpm',
...
function printMcpSummary(selectedMcpIds: string[]) {
  ...
  const configuredServers = selectedMcpIds.map(id => {
    const server = MCP_SERVERS.find(s => s.id === id);
    if (!server) return { server, category: undefined };
    const category = server.category;
    return { server, category };
  });

  // Group by category
  const byCategory = configuredServers.reduce((acc: Record<string, Array<{ server: typeof MCP_SERVERS[number]; category?: string }>>, { server, category }) => {
    if (!category) return acc;
    if (!acc[category]) acc[category] = [];
    acc[category].push({ server, category });
    return acc;
  }, {});
