...
1. **Initialize steering**: `@spec /configure`
...
| `tech.md` | Technology stack, versions, dependencies | "Next.js 14, PostgreSQL, Prisma ORM" |
| `architecture.md` | System architecture, patterns, boundaries | "Hexagonal architecture with ports and adapters" |
| `conventions.md` | Coding standards, naming patterns | "Use kebab-case for files, PascalCase for components" |
| `testing.md` | Testing strategy, coverage requirements | "Unit tests for business logic, integration tests for APIs" |
...
View tasks in the **Tasks** sidebar view, then use **Copilot Agent mode**:

1. Open your spec's `tasks.md` file
2. Ask Copilot to implement specific tasks:
   - "Implement task 1.1 from the spec"
   - "Implement all pending tasks in phase 1"

**💡 Tip:** Add `tdd` to the tasks command (`@spec /tasks tdd`) for test-first approach.
...
4. **Implementation Phase** - Write code using Copilot Agent mode
   - Open `tasks.md` as context
   - Ask Copilot: "Implement task 1.1"
...
**Step 3: Implement with Copilot Agent mode**
- Open `tasks.md` as context
- Ask Copilot: "Implement the fix tasks"
...
| `/configure` | Initialize or update steering documents | `@spec /configure` |
...
### Steering View
- **Documents**: product.md, tech.md, architecture.md, conventions.md, testing.md
...
**To implement tasks:** Use Copilot Agent mode with `tasks.md` as context.
